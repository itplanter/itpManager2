...
public class ReportPanel extends JPanel implements ActionListener{
	...
	public  static GDCheck tempDisp=null;
	public  static GDCheck waterDisp=null;
	public  static GDCheck illumDisp=null;
	/**
...
//		tmpDisp.setAction(this.acceptEnableAction());// Action when checked
		...
		int id=0;
		if(s!=null) id=(Integer.valueOf(s)-35)/5;
		...
		for(int i=0;i<5000;i+=100) label[i/100]=String.valueOf(i);
		illumDisp.setSelectionItems(label);
		String si=Files.getIlluminationWarningLevel();
		int idi=(Integer.valueOf(si)-0)/100;
	...
	*/
	...
	public void update(Observable arg0, Object arg1) {
		...
		if(message.contains("警告温度")){
			...
			String msg="警告温度 "+t[2]+System.getProperty("line.separator");
		...
		if(message.contains("警告水位 ")){
			...
			String msg="警告水位 "+t[2]+System.getProperty("line.separator");
		...
		if(message.contains("警告照度")){
			...
			String msg="警告照度 "+t[2]+System.getProperty("line.separator");
...
//				if(Sensor.temperatureReport==true||Sensor.illuminationReport==true||Sensor.waterlevelReport==true){
				...
				if(Sensor.temperature > Sensor.getTemperatureWarningLevel()){
					...
					String msg="Temp Warnig over "+String.valueOf(Sensor.temperature);
				...
				if(Sensor.illumination < Sensor.getIlluminationWarningLevel()){// and check Light statue
					...
					String msg="Illum Warnig under "+String.valueOf(Sensor.illumination);
					sendMail.addMessageText(msg);
					sendMail.setTitleText(msg);
				} 
				if(Sensor.waterlevel < Sensor.getWaterWarningLevel()){// and check pump status
					Sensor.waterlevelReport=true;
					String msg="Water Warning under　"+String.valueOf(Sensor.waterlevel)+" %%";
					sendMail.setTitleText(msg);
